...
colSpec <- cols(SampleID = col_character(), 
                ...
                batch = col_character(),
                no.DNMs = col_integer())
samples_c1 <- read_delim(file = here("compendium/data/samples_c1.tsv"), delim = "\t", col_types = colSpec)
samples_c2 <- read_delim(file = here("compendium/data/samples_c2.tsv"), delim = "\t", col_types = colSpec)
samples_c3 <- read_delim(file = here("compendium/data/samples_c3.tsv"), delim = "\t", col_types = colSpec)
samples_c4 <- read_delim(file = here("compendium/data/samples_c4.tsv"), delim = "\t", col_types = colSpec)
samples_c5 <- read_delim(file = here("compendium/data/samples_c5.tsv"), delim = "\t", col_types = colSpec)
...
allEstimates <- 
  bind_rows(
    ...
    samples_c2 %>% calcRelativeVarCorsWithCi() %>% mutate(set = "cohort2"),
    samples_c3 %>% calcRelativeVarCorsWithCi(withBatch = FALSE) %>% mutate(set = "cohort3"),
    samples_c4 %>% calcRelativeVarCorsWithCi() %>% mutate(set = "cohort4"),
    samples_c5 %>% calcRelativeVarCorsWithCi() %>% mutate(set = "cohort5")
  ) 
...
familyEstimates <-
  ...
  mutate(includesBatch = c(TRUE, TRUE, FALSE, TRUE, TRUE)) %>%
