...
estimateSd <- familyEstimates %>% filter(set %in% c("cohort3","cohort4")) %>% with((upper.total-relVar.total)/1.96)
Hmisc::wtd.var(
  x=estimateSd^2,
  w=familyEstimates$no.familys[familyEstimates$set %in% c("cohort3","cohort4")]*familyEstimates$no.childrenPerFamily[familyEstimates$set %in% c("cohort3","cohort4")]
)
...
# volgens deze pagnia's kom ik op een pooled variance van 0.018, dus een bovengrens van 0.084
```
...
write_delim(allEstimates,
            here("dnmvar-compendium/analyses/varianceComponentEstimations/allEstimates.tsv"),
            delim = "\t")

write_delim(familyEstimates,
            here("dnmvar-compendium/analyses/varianceComponentEstimations/familyEstimates.tsv"),
            delim = "\t")
...
samples_c3b %>% 
  ...
  mutate(includesBatch=FALSE, region="unique") %>% 
  cbind(familyStats(samples_c3b))
