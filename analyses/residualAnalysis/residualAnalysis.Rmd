...
ggplot(agg_res, aes(x=dataset, y=delta, group=paste(dataset, set))) +
  ...
  ggsignif::geom_signif(annotations = c("p=0.56", "p=0.38", "p=0.055"),  # adapt these values manually
                        y_position = c(30, 40, 55), xmin=c(0.8,1.8,2.8), xmax=c(1.2,2.2,3.2))
```
...
samples_c3 %>% 
  mutate(resi = residuals(simpMod)) %>% 
  mutate(proband = if_else(grepl("p", SampleID), "ASD-affected", "unaffected sibling")) %>% 
  ggpubr::ggboxplot(x="proband", color="proband", y="resi", add="jitter") + 
  ggpubr::stat_compare_means(comparison = list(c("ASD-affected", "unaffected sibling"))) + 
