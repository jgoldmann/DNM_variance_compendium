...
colSpec <- cols(SampleID = col_character(), 
                Chr = col_character())
mutations_c1 <- read_delim(here("compendium/data/mutations_c1.tsv"), delim = "\t", col_types = colSpec)
mutations_c2 <- read_delim(here("compendium/data/mutations_c2.tsv"), delim = "\t", col_types = colSpec)
mutations_c3 <- read_delim(here("compendium/data/mutations_c3.tsv"), delim = "\t", col_types = colSpec)
  ...
  list(
    ...
    mutations_c2 %>% 
      ...
      complete(nesting(SampleID, 
               ...
               fill = list(n=0)),
    mutations_c3 %>% 
      full_join(samples_c3, by = "SampleID") %>% 
      group_by(SampleID, fathersAgeAtConceptionInYears, mothersAgeAtConceptionInYears, Substitution) %>% 
      dplyr::count() %>% 
      ungroup() %>% 
      filter(!is.na(Substitution)) %>%
