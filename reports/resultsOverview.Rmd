    ...
    poisDens <- 
      foreach(subs = unique(ageMdlFrame$substitution),
              .combine = bind_rows) %do% {
                tibble(
                  no.muts = 0:75,
                  dens = getDensitySums(subs, mdlFrame = ageMdlFrame, mdls = mdlList)/816,
                  substitution = subs)
              }
    countFrame <- 
      ageMdlFrame %>%
      ...
      dplyr::rename(no.muts = n,
                    observed = nn) %>%
      full_join(poisDens %>%
                  mutate(predicted = round(dens*816))%>% #rounding the estimates
                  select(-dens), 
  ...
  foreach(countFrame = countFrameList,
          .combine = bind_rows) %do% {
            countFrame %>% 
              ...
              group_by(substitution) %>% 
              group_modify(~data.frame(wilcox.p=wilcox.test(cases~set, data=.x)$p.value,    #test for difference in median
                                       ...
                                       ansari.p = ansari.test(cases~set, data=.x)$p.value,  #         difference in scale parameters
